...
\begin{equation}
     ...
     \sum_{j\ne i}^{N}\left(
          a_{ij}-\frac{g_i^{IN}g_j^{OUT}}{L}  \right)\delta(C_i,C_j)
\end{equation}
...
The difference from the regular networks of this model is that the presence of a few random long-distance connections allows the modules to exchange information efficiently at a global level, without destroying the local modular structure.
...
What happens if the the Nyquist frequency is not high enough? The frequency components of the sampled signal that are higher than $\omega_n$ do not vanish, but they ``fold'' within the basic spectrum. These high frequencies take on a false identity (precisely called \textbf{alias}) and appear as lower frequencies than they actually are, following the formula:\begin{equation}
...
Now let's see some measure of \textit{dispersion}, that are the opposite of central tendency. The \textbf{Average Rectified Value} (ARV) is the following value:\begin{equation}
...
that are blocked. The band-reject filter is also called \textbf{notch-filter}. The transition band is between. A fast \textbf{roll-off }means that the transition band is very
narrow.
