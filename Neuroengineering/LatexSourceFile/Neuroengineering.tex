...
of a causality link directed from signal $j$ to signal $i$. As a recap:\begin{itemize}
     ...
     \item The Granger causality index $G_{ij}$ is directed and bivariate, it's not defined in the frequency domain. 
...
We recap the properties of a graph under two labels:
...
\noindent Where do the names Finite Impulse Response and Infinite Impulse Response come from? These names refers to the filter output when the signal in input is a unit impulse (a signal zero everywhere except on zero, that is 1):\begin{equation}
     x(n)=\begin{cases}
          1 \text{ if }n=0\\ 
          0 \text{ instead}
     \end{cases}.
