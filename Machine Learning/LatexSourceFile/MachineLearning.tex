...
We focus now on the concept learning (binary classification), let's consider a target function\begin{equation}
    c:X\rightarrow \{X,Y\}
\end{equation}
let $D\subset X\rightarrow\{0,1\}$ to be the sample set\begin{equation}
...
\begin{definition}
    If $X$ is a set, $\mathcal D$ is a probability distribution on $X$ if\begin{eqnarray}
        \forall x\in X, \ \ \mathcal D(x)\in [0,1]
    \end{eqnarray}
    and \begin{align*}
        &\sum_{x\in X}\mathcal D(x)=1 \ \text{if $X$ is countable}\\
        &\int_X\mathcal D(x)dx=1 \ \text{if $X$ is uncountable}
    \end{align*}
\end{definition}
\begin{definition}
