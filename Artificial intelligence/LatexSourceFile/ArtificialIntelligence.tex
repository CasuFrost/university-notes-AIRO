...
\end{equation} 
An action $a$ is optimal if it maximizes the expected value of $M$, given the sequence $P$ and the knowledge $K$. An agent is rational if he always chooses the optimal action. More specifically, an agent consists in two components:\begin{itemize}
    \item an architecture which provides an interface to the environment
    \item a program executed on that architecture.
\end{itemize}
There are some limitations that we aren't considering, such as the fact that determining the optimal choice could take too much time or memory on the architecture.

\subsection{Types of Agents}
There are different kinds of agents, a \textbf{Table Driven Agent} is the simplest form of agent architecture. It's essentially a look-up table that maps every possible sequence of percepts (what the agent has sensed so far) to a corresponding action the agent should take. Its behavior can be summarized in the algorithm \ref{alg:table_agent}.
...
An environment can be classified in terms of different attributes:\begin{itemize}
    ...
    \item An environment can be \textbf{static} or \textbf{dynamic}, if it does not change, but the agent's performance
...
\end{center}
Starting from Zurigo, we would like to find a route to Zagabria. We have an initial state (Zurigo), and we have to apply actions (drive) to be performed to reach the goal state (Zagabria). Another example is the following, we want to solve the tiles-puzzle game, shown in figure \ref{img:tiles}, to reach the right state, starting from the left one, the actions to perform are the move of the tiles. A performance measure could be to minimize the summed-up action costs.
...
\begin{definition}
    We have a \textbf{declarative description} of the problem if is described by the following sets:
\end{definition}
\begin{itemize}
...
\noindent When we analyze a search algorithm, we are interested in various properties\begin{itemize}
...
(Measured in states).
    ...
    \item \textbf{Goal depth}: the number $d$ of action required to reach the shallowest (nearest to the initial state) goal state.
\end{itemize}
...
About the space complexity, this method stores only a single path of actions, from the root to a leaf node, since once a
 ...
 This method is equivalent to the well known Dijkstra's algorithm. We expand the node with the lowest path cost $g(n)$, the frontier is ordered by the path cost, with the lowest first. It differs from the BFS since a test is added to check if a better path is found to a node currently
 on the frontier.

 \begin{algorithm}
    \caption{Uniform-Cost Search}\label{alg:Uniform-Cost-Search}
    \begin{algorithmic}
    \Require $problem$
    \State $node\leftarrow$\texttt{InitialState()}
...
\hline
Criterion                                                  & BFS                                                                 & \begin{tabular}[c]{@{}c@{}}Uniform\\ Cost\end{tabular}                                             & DFS      & \begin{tabular}[c]{@{}c@{}}Depth\\ Limited\end{tabular} & \begin{tabular}[c]{@{}c@{}}Iterative\\ Deepening\end{tabular}         \\ \hline
...
\begin{algorithm}
    ...
    \State $population$ = $population\cup child$
    \State remove from $population$ the individual with lowest fitness
    \EndFor 
...
\begin{itemize}
    ...
    \item Crossover helps if and only if substrings are meaningful components: we combine large blocks of letters that have evolved independently to perform useful functions.
...
This is the canonical algorithm for solving games, by computing an optimal strategy. Remember that the player $Max$ attempts to maximize the utility function $u(s)$ of the terminal state that will be reached during play, when $Min$ attempts to minimize it.\bigskip 
...
The left branch leads to a utility at least $n$, the right branch have one sub-branch with utility value $m<n$, so we can say for sure that the left tree will not provide values for the utility grater than $m$ (since it's the $Min$ turn), at this point we can discard the right branch (and stop checking for the other nodes) and continue with the left one.\bigskip

\noindent We consider two additional variables $\alpha,\beta$ defined on each node during the search, such that:\begin{itemize}
...
The Alpha-Beta search have two issues\begin{itemize}
    ...
    \item it doesn't explore so much in problems with a large branching factor such as \textit{Go}.
\end{itemize}
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{Playout Policy:} During a playout, a policy is used to bias move selection toward ''good'' moves rather than purely random ones. For complex games such as Go, these policies are often learned via \textbf{self-play} using neural networks.
...
There exists CSP solvers, generic algorithms that find an assignment for constraints problems, by taking in input a constraint network as the generic language to describe the problem. The core exercise in this context is to model a problem with a constraint network.
\begin{definition}
...
\section{Inference}
Given a constraint network, we would like to obtain an equivalent network with more constraints, without losing any feasible solution, in such case, we are giving a tighter description of the problem, and we obtain a network with a smaller number of
...
\begin{algorithm}
    ...
    \State \color{sapienza}$\gamma'=$ Inference$(\gamma')$\color{black}
    ...
    \State $a'=a\cup \{v=d\}$
    \State $D'_v=\{d\}$
    \State $a''=$ BacktrackingWithInference($\gamma'$,$a'$)
    \If{$a''$ is not inconsistent} \State\Return $a''$\EndIf
    \EndFor
    \State\Return inconsistent
    \end{algorithmic}
\end{algorithm}
With the function \color{sapienza}Inference \color{black} we denote any procedure deriving a tighter equivalent network. One simple inference procedure called \textit{forward checking} is described in Algorithm \ref{alg:FWCH}.\bigskip 
...
\begin{algorithm}
    ...
    \State $D'_u=\{d \ | \ d\in D_u, (d,d')\in C_{uv}\}$\greenText{/*We restrict the domain to the admissible assignments*/}
    \State $D_u\leftarrow D'_u$ 
    \EndFor 
    \EndFor 
     \State\Return $\gamma$
    \end{algorithmic}
...
The forward checking Algorithm \ref{alg:FWCH} tightens the constraints without ruling out any solutions, it guarantees to deliver an equivalent network. The computation can be done incrementally, instead of performing the first loop, we may run only the second loop every time a new assignment $a(v)=d'$ is added.\begin{itemize}
...
We can define an inference procedure that aims to make the network $\gamma$ arc consistent, by removing variable domain values. Let's say that AC($\gamma$) is a procedure that makes $\gamma$ arc consistent, in such case:\begin{itemize}
    ...
    \item AC($\gamma$) is more powerful (more general) than the forward checking, i.e.:\begin{equation}
...
\begin{algorithm}
    ...
    \State $D_u=D_u\backslash\{d\}$ \greenText{ /*We remove the value that makes $v$ NOT arc consistent with $u$*/}
    \EndIf
    \EndFor 
    \State\Return $\gamma$
    \end{algorithmic}
...
\begin{algorithm}
    ...
    \end{algorithmic}
\end{algorithm}
\noindent\textbf{Algorithm Breakdown}\begin{itemize}
    \item At the beginning, we create a set $M$, in which we add all the pair of variables to check (in both directions), this is due to the fact that, if $(u,v)\in M$, we want to make $u$ arc consistent to $v$, but this relation is not commutative, so to make $v$ arc consistent to $v$ we also add $(v,u)\in M$.
    \item in te main \textbf{while} loop we remove a pair $(u,v)$ 
    \item we \texttt{Revise} $u,v$: if for a value of $u$ there is no valid option in $v$, that value is deleted from the domain of $u$, in this way, $u$ becomes arc consistent with $v$.
    \item since we reduced the domain of $u$, for all the other variables $w\ne v$, it might be that some values that were valid for the constraints, aren't valid anymore, so for each $w\ne v$ we add $(w,u)$ in $M$.
\end{itemize}
\begin{theorem}
    Let $\gamma$ to be a constraint network with $m$ constraints and maximal domain size $k$. The algorithm AC3 showed in \ref{alg:AC3} have a time complexity of $O(mk^3)$.
\end{theorem}
...
\end{center}
Observe how this graph is composed by two connected sub-graph, since the variable $T$ (Tasmania) is disconnected from the other variables. The idea is to separate a network in function of his connected sub-graphs.  
\begin{theorem}
...
\begin{definition}
    ...
    if, all the models $I$ of $KB$ (the assignments that makes true all the formulas in $KB$ simultaneously), are models of $\varphi$. 
...
\begin{definition}
    ...
    \end{equation}
\end{definition}
It is important to know that \textbf{every formula} can be expressed in CNF or DNF, i.e. for each formula $\varphi$, there exists an equivalent formula $\psi$ that is a CNF or a DNF. It is possible to construct the equivalent CNF (or DNF) from $\varphi$ by exploiting the equivalences such as\begin{itemize}
...
We can construct a simple algorithm that, given a CNF formula $\psi$, tests if is satisfiable or not, by trying to derive from $\psi$ new formulas: If it finds a derived  formula that is unsatisfiable then $\psi$ is unsatisfiable.\bigskip 
...
\begin{definition}
    An \textbf{inference rule} is a rule that derive a new formula from a given one, the \textbf{resolution rule} is the following:\begin{equation}
...
\end{itemize}
In practice, if we have two clauses $C'_1=C_1\dot\cup\{l\}$ and $C'_2=C_2\dot\cup\{\lnot l\}$ in $\Delta$, we can derive a disjunction (i.e. a new clause) $\varphi = C_1\cup C_2$. The two starting clauses $C'_1$ and $C'_2$ are called parent clauses.\begin{quote}
...
\begin{definition}\label{Def:Soundness}
    An inference rule satisfy the \textbf{Soundness Property} if, we can derive $D$ from $\Delta$ by the rule and it holds that $\Delta\models D$ (the CNF $\Delta$ entails $D$).
\end{definition}
...
The resolution algorithm takes as an input a knowledge base $KB$ and a formula $\varphi$ and guess if $KB\models \varphi$ by the following steps:\begin{enumerate}
    \item transform $KB$ in a CNF 
    \item write the CNF as a set of clauses $\Delta$, by adding to $\Delta$ also the clause $\{\lnot \varphi\}$
...
That is, if $\Delta$ contains parent clauses of the form $C\dot\cup\{\lnot l\}$ and $\{l\}$ the rule
 ...
 Does the UP rule have the soundness property of Definition \ref{Def:Soundness}? Yes, we have to show that if $\Delta'$ can be derived from $\Delta$ then $\Delta\models \Delta'$, this is true since any derivation made by unit resolution can also be done by the full resolution, which we already know has this property.
...
\subsection{Conflict Analysis and Clause Learning}
 ...
 In tree resolution, every time a clause is needed, it must be derived from scratch because results aren't stored, this leads the DPLL to ''make the same mistakes over and over again'', leading to proofs that are exponentially longer than they need to be. Modern SAT solvers use Clause Learning (CDCL). When the solver hits a conflict, it analyzes the cause, creates a new clause to ''remember'' that mistake, and adds it to the database so it never explores that specific failing path again.\bigskip 
    ...
    \begin{itemize}
        ...
        \item the edges $E(G^{impl})$ are the following:\begin{itemize}
            \item when a clause $C=\{l_1\dots l_k,l'\}\in \Delta$ becomes unit with the implied literal $l'$, there will be the edges\begin{align}
                &\lnot l_1\longrightarrow l'\\ &\vdots\\
                &\lnot l_k\longrightarrow l'
            \end{align}
            \item when  a clause $C=\{l_1\dots l_k\}$ becomes empty there will be the edges\begin{align}
                &\lnot l_1\longrightarrow \square_C\\ &\vdots\\
                &\lnot l_k\longrightarrow \square_C
            \end{align}
        \end{itemize} 
    \end{itemize}
If the clause $\{l_1\dots l_k,l'\}$ became unit: $\{l'\}$, it means that the other literals $l_1\dots l_k$ have been set to false\footnote{
    we recall that, if a literal $l$ is set to false, we don't mean that $l=0$, but that the truth value of the literal $l$ (that might be a negated atom) is 0, so if $l=\lnot P$ with $P$ atom, then $l=0\implies P=1$.
}, and $l'$ since is the only literal in the clause (that we remind that is included in a conjunction) must be true.
...
\}
    ...
    \item Second step: We apply the splitting rule on $P$, creating two branches, one with $P=0$ and the other with $P=1$, we continue with the branch with $P=0$. The literal $\lnot P$ becomes a choice literal so there will be the vertex $\lnot P$ in the graph.
    \begin{equation}
        \{\{P,Q\},
\{\lnot P,\lnot Q\},
\{P,\lnot Q\}
...
An implication graph cannot have cycle because it keeps track of chronological behavior
 ...
 \begin{definition}
    Let $\Delta$ to be a set of clauses and  $G^{impl}$ the implication graph relative to a branch of the DPLL algorithm, a \textbf{conflict graph} $G^{conf}$ is a sub-graph of $G^{impl}$ such that:\begin{itemize}
        ...
        \item if $l'$ is a vertex in $G^{confl}$, then, all the parents of  $l'$ that are vertices $\lnot l_i$ with the arc $(\lnot l_i,l')\in E(G^{impl})$ are also vertices in $G^{conf}$.
...
\end{itemize}
 ...
 \begin{definition}[Ground Atom Satisfaction]
    Let $\Sigma$ be a signature and $I$ a $\Sigma$-interpretation. We say that $I$ satisfies a ground atom $P(t_1, \dots, t_n)$, written $I \models P(t_1, \dots, t_n)$, if and only if $(t_1^I, \dots, t_n^I) \in P^I$. We also call $I$ a model of $P(t_1, \dots, t_n)$.
$$ 
...
\end{definition}
 \begin{center}
    \includegraphics[width=0.3\textwidth ]{images/unifier.eps}
\end{center}
\begin{proposition}
...
\end{itemize}
We remind that to decide whenever $KB\models \varphi$ we can check the satisfiability of $\psi = KB\cup\{\lnot\varphi\}$. If this is unsatisfiable then $KB\models \varphi$. So the idea is\begin{itemize}
...
\end{definition}
We denote $\mathcal{R}_{FactBin}$ the calculus with the factoring rule and the binary FOL resolution, and we denote $\mathcal{R}_{Full}$ the calculus with the full FOL resolution.
...
\end{lemma2}
\begin{theorem}
    The calculus $\mathcal{R}_{Full}$ and the calculus $\mathcal{R}_{FactBin}$ are both sound and complete.
\end{theorem}
A sketch of the proof is presented through previous intermediate results:\begin{itemize}
    \item Any set $\theta$ of FOL formulas can be represented in clausal form $\Delta$
...
\begin{definition}[FDR State Space]
    Let $\Pi=(V,A,c,I,G)$ to be an FDR planning task, the \textbf{task space} of $\Pi$ is the labeled transition system $\Theta_\Pi=(S,L,c,T,I,S^G)$ where\begin{itemize}
        ...
        where\begin{itemize}
            ...
            $$
            s\quadBrack{a}=\text{undefined} \text{ if }a\notin A[s]
            \ \ \text{ and } \ \ 
            s\quadBrack{a}(v)=\begin{cases}
...
Although the STRIPS language is still used for historical convenience, modern planning systems (such as Fast Downward) prefer to translate everything internally into FDR for reasons of practical efficiency. The FDR format is superior because it reduces unnecessary states, allows you to better map dependencies via causal graphs (Section \ref{sec:CausalGraph}), and facilitates the creation of more powerful heuristics, offering a much more natural and compact way of modeling reality than the rigid Boolean variables of STRIPS.
...
\end{lstlisting}
you are defining the objects \texttt{ obj1 obj2 ... objn } and declaring that they are of the type \texttt{ root\_obj}. If not specified, each object inheritance the root default type \texttt{object}.
...
\section{Causal Graph}\label{sec:CausalGraph}
\begin{definition}
...
\end{center}
So, we defined two types of arc, the class 1 (precondition-effect) and the class 2 (effect-effect). In the causal graph, an ''arc'' of class 2 is in fact a couple of directed arc forming a cycle between two variables,  occur whenever an action
 ...
 A causal graph doesn't depend on the initial state $I$ or the goal state $G$, this is a main weakness of causal graphs, they capture only the
...
Such variable $v$ is called ''servant'', thanks to its connected and invertible DTG, it can always go wherever it is needed. The optimal plan for $\Pi'$
 ...
 \end{lemma2}
 As we already said, if the plan for $\Pi'$ is optimal, is not directly implied that also the plan for $\Pi$ is optimal.
 \begin{theorem}
 ...
 $$s\quadBrack{a}=\begin{cases}
                ...
                s(v) \text{ if }v\notin  V[eff_a]
            \end{cases}$$
\begin{definition}[FDR Regression]\label{FDR_regr}
...
\end{definition}\begin{enumerate}[label=(\roman*)]
    \item $eff_a\cap g\ne \emptyset$
...
\end{enumerate}
\begin{proposition}
    The regression of Definition \ref{FDR_regr} satisfies the condition of Equation \eqref{regr_cond}.
\end{proposition}
...
\end{equation}
\begin{proposition}
    The regression of Definition \ref{STRIPS_regression} satisfies the condition of Equation \eqref{regr_cond}.
\end{proposition}
...
The reader is advised to review the definitions of a heuristic function given in Section \ref{InformedSearch}.
...
\noindent This algorithm \ref{alg:EHC} is still not optimal:\begin{itemize}
    ...
    \item It is only complete if it is guaranteed that no such states exist and a goal is always reachable with a lower heuristic value.
\end{itemize}
...
\end{proposition}
\textit{Proof}: Let $m=|P|$, in that case since $g\subseteq P$ we have $|g|\le m$ so $h^m=r^*$.\hfill$\blacksquare$
...
The algorithm \ref{alg:mPG} is the generalization for a generic $m\in \N$.
...
We will present some formal definition and state some
...
\begin{definition}
    Let $\Pi=(P,A,c,I,G)$ be a STRIPS planning task with state space $\Theta_\Pi=(S,A,c,T,I,G)$. The \textbf{optimal delete relaxation heuristic} $h^+$ for $\Pi$ is the function $h^+:S\rightarrow\R^+\cup\{0,\infty\}$ where $h^+(s)$ is defined as the cost of an optimal relaxed plan for $s$.
\end{definition}
...
We will define the \textbf{best supporters functions} for the additive and max heuristic, that are two functions which, for every fact $p\in P$  returns an action that is deemed to be the cheapest achiever of $p$
 ...
 simply selecting an action achieving $p$ that minimizes [$c(a)$ plus the
