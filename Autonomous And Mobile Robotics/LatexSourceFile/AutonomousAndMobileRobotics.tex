...
\end{boxdefinition}
Let's see an example, we consider a disk robot that moves on a plane, that can also rotate along its main axis to point towards a desired direction, shown in Figure \ref{disc_tick}.
\begin{figure}[H]
    \centering
...
\end{equation}
We have seen in the previous courses of robotics that the number of generalized coordinates of an  $n$-link serial manipulator is exactly $n$.
...
\end{figure}
This representation also presents a problem: The distance between two configurations in $\mathcal C$ does not represent the true distance between two postures in the workspace: Two very distant configurations $\mathbf a$ and $\mathbf b$ in $\mathcal C$ can be mapped to two very close postures in $W$:
\begin{figure}[H]
    \centering
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.4\textwidth ]{images/torus.eps}
